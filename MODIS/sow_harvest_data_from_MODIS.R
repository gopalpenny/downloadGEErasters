...
library(sf)
library(tidyverse)
haryana_shp_path <- '/Users/gpenny/Library/CloudStorage/GoogleDrive-gopalpenny@gmail.com/My Drive/_Research/Research projects/G_D/GEE_Div/GEE_DIV_MODIS_EVI_haryana/haryana_shp/haryana.shp'
subdists_india <- read_sf(haryana_shp_path)
subdists_punjab <- subdists_india #%>% dplyr::filter(STATE_UT == 'Haryana')
subdists_punjab_vect <- vect(subdists_india) #, crs = crs(year_sowday, proj = TRUE)

# punjab_evi_path <- '/Users/divya/Library/CloudStorage/GoogleDrive-divysolo@umich.edu/Other computers/My MacBook Air/Project/Work/Postdoc/First_data/Spatial/Sow_date/Haryana_evi_200_23'
punjab_evi_path <- '/Users/gpenny/Library/CloudStorage/GoogleDrive-gopalpenny@gmail.com/My Drive/_Research/Research projects/G_D/GEE_Div/GEE_DIV_MODIS_EVI_haryana/GEE_DIV_MODIS_EVI_haryana_evi'
...
rast_w_projection <- rast(rastlist[1])
subdists_punjab_vect <- project(subdists_punjab_vect, rast_w_projection)


out_dir <- file.path(punjab_evi_path, "sowdates_2000_3")
...
set.seed(101)
px_samples <- sample(1:(nrow(rast_w_projection)*ncol(rast_w_projection)), size = 40, replace = FALSE)
...
for (year_i in years) {
  ...
  if (year_i == years[1]) {
    subdist_sowday_df <- data.frame(SUB_ID = subdists_punjab$NAME, sowday = subdist_sowday, year = year_i)
  } else {
    subdist_sowday_df <- subdist_sowday_df %>%
      dplyr::bind_rows(data.frame(SUB_ID = subdists_punjab$NAME, sowday = subdist_sowday, year = year_i))
  }
...
write_csv(subdist_sowday_df, file.path(out_dir,"subsdist_median_sowday_2000_1.csv"))
st_write(subdists_punjab, file.path(out_dir,"dist_punjab_sowday_2000_2021_test.shp"))